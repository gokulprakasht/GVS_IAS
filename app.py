...
with st.sidebar:
    ...
    _wl_name  = _wl.get("brand_name",  "IAS v9.0")
...
# ════════════════════════════════════════════════════════════════
    ...
    with _ab3:
        ...
        if _ab_st:
            st.markdown('<div style="background:rgba(0,201,167,0.08);border:1px solid rgba(0,201,167,0.2);border-radius:6px;padding:8px 12px;font-size:11px;color:#00C9A7">' + ' · '.join(_ab_st) + '</div>', unsafe_allow_html=True)
        else:
            st.caption("Load .eml or fill CV + JD in Step 1 to start")
    st.divider()
